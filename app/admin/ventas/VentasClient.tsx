...
import {
  MdCalendarMonth, MdWarningAmber, MdAttachMoney, MdCheckCircle, MdGroup,
  MdChevronLeft, MdChevronRight, MdRefresh, MdPrint,
} from 'react-icons/md'
import type { PaymentMetrics } from '@/app/admin/_actions/payments'
import type { FollowupMetrics } from '@/app/admin/_actions/followups'
import type { getRetentionStats } from '@/app/admin/_actions/retention'
import type { ExecutiveData } from './_data'
import { peso, percentage, shortPeso, formatMonthParam, monthLabel, isCurrentMonth } from './_utils'

type RetentionStats = Awaited<ReturnType<typeof getRetentionStats>>

export type TabKey = 'finanzas' | 'comercial' | 'retencion' | 'operacion'

const TABS: { key: TabKey; label: string; desc: string }[] = [
  { key: 'finanzas', label: 'Finanzas', desc: 'Cobranza e ingresos' },
  { key: 'comercial', label: 'Comercial', desc: 'Embudo y captación' },
  { key: 'retencion', label: 'Retención', desc: 'Alumnos activos y en riesgo' },
  { key: 'operacion', label: 'Operación', desc: 'Instructores y salones' },
]
...
export default function VentasClient({
  data, retentionStats, pm, fm,
  billedMonth, cobradoMonth, pendienteTotal, pendienteCount, cobradoPct, salesGrowthLabel, alertas,
  refMonth, initialTab,
}: Props) {
  const [tab, setTab] = useState<TabKey>(initialTab ?? 'finanzas')
  const [isPending, startTransition] = useTransition()
  const router = useRouter()

  function goToMonth(next: Date) {
    startTransition(() => {
      router.push(`/admin/ventas?month=${formatMonthParam(next)}&tab=${tab}`, { scroll: false })
    })
  }

  function handleTabChange(key: TabKey) {
    setTab(key)
    router.replace(`/admin/ventas?month=${formatMonthParam(refMonth)}&tab=${key}`, { scroll: false })
  }
  ...
  return (
    <div className="space-y-6 page-animate ventas-print-root">
      {/* ── Header ─────────────────────────────────────────── */}
      <section className="rounded-[28px] border border-white/10 bg-[#0b0b0b] px-5 py-5 lg:px-8 lg:py-6 shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3 print:hidden">
            <div className="inline-flex items-center gap-1 rounded-2xl border border-white/10 bg-white/[0.03] px-2 py-2 text-sm text-white/85">
              <button
                type="button"
                onClick={() => goToMonth(new Date(refMonth.getFullYear(), refMonth.getMonth() - 1, 1))}
                className="grid h-7 w-7 place-items-center rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors"
                aria-label="Mes anterior"
              >
                <MdChevronLeft className="h-4 w-4" aria-hidden="true" />
              </button>
              <span className="flex items-center gap-2 px-2">
                <MdCalendarMonth className="h-4 w-4 text-white/55" aria-hidden="true" />
                {monthLabel(refMonth)}
              </span>
              <button
                type="button"
                onClick={() => goToMonth(new Date(refMonth.getFullYear(), refMonth.getMonth() + 1, 1))}
                disabled={isCurrentMonth(refMonth)}
                className="grid h-7 w-7 place-items-center rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors disabled:opacity-25 disabled:hover:bg-transparent"
            ...
            <button
              ...
              disabled={isPending}
              className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/75 hover:text-white hover:border-white/20 transition-colors disabled:opacity-50"
            >
              <MdRefresh className={`h-4 w-4 ${isPending ? 'animate-spin' : ''}`} aria-hidden="true" />
              {isPending ? 'Actualizando…' : 'Recargar'}
            </button>
            <button
              type="button"
              onClick={() => window.print()}
              className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/75 hover:text-white hover:border-white/20 transition-colors"
            >
              <MdPrint className="h-4 w-4" aria-hidden="true" />
              Exportar PDF
            </button>
          </div>
        </div>
        ...
        <div className="rounded-[24px] border border-[#ff7a00]/20 bg-[#0d0a07] p-5 shadow-[0_0_40px_rgba(255,122,0,0.06)]">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <AlertItem
              ...
              tone="orange"
              href="/admin/leads?view=lista&estado=pending"
            />
            <AlertItem
      ...
      {tab === 'finanzas' && (
        <FinanzasTab
          data={data} pm={pm}
          billedMonth={billedMonth} cobradoMonth={cobradoMonth}
        />
      )}
      {tab === 'comercial' && <ComercialTab data={data} pm={pm} />}
      {tab === 'retencion' && <RetencionTab data={data} retentionStats={retentionStats} fm={fm} />}
      {tab === 'operacion' && <OperacionTab data={data} />}
    </div>
  )
}
...
function FinanzasTab({
  data, pm, billedMonth, cobradoMonth,
}: { data: ExecutiveData; pm: PaymentMetrics; billedMonth: number; cobradoMonth: number }) {
  return (
    <div className="space-y-4">
      {pm.hasRealData && (
        <ExecutiveCard title="Cobranza del Mes">
          <div className="grid gap-6 lg:grid-cols-[1fr_1fr_1fr_1px_1fr]">
            <CobranzaBar label="Cobrado" pct={pm.cobradoPct} amount={cobradoMonth} color="green" />
            <CobranzaBar label="Pendiente" pct={pm.pendientePct} amount={pm.pendienteTotal} color="orange" />
            <CobranzaBar label="En Mora" pct={pm.moraPct} amount={pm.overdueTotal} color="red" />
        ...
        <ExecutiveCard title="Ventas por método de pago">
          {pm.byMethod.length > 0 ? (
            <div className="grid gap-5 lg:grid-cols-[220px_1fr]">
              <DonutChart total={pm.byMethod.reduce((s, m) => s + m.value, 0)} items={pm.byMethod} centerLabel="Total" />
              <LegendList items={pm.byMethod} total={pm.byMethod.reduce((s, m) => s + m.value, 0)} compact />
            </div>
          ) : (
            <EmptyState text="Sin pagos confirmados con método registrado todavía." />
          )}
        </ExecutiveCard>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1.1fr]">
        ...
        >
          {data.recentSales.length > 0 ? (
            ...
            <EmptyState text="Sin matrículas convertidas recientes." />
          )}
        </ExecutiveCard>
      </div>
    </div>
...
function ComercialTab({ data, pm }: { data: ExecutiveData; pm: PaymentMetrics }) {
  return (
    <div className="space-y-4">
        ...
        {pm.byPlan.length > 0 ? (
          <div className="grid gap-5 lg:grid-cols-[220px_1fr]">
            <DonutChart
              ...
              centerLabel="Cobrado"
            />
            <LegendList items={pm.byPlan} total={pm.byPlan.reduce((s, p) => s + p.value, 0)} />
          </div>
        ) : (
...
function RetencionTab({
  data, retentionStats, fm,
}: { data: ExecutiveData; retentionStats: RetentionStats; fm: FollowupMetrics }) {
  return (
    <div className="space-y-4">
      <ExecutiveCard
        title="Retención Académica"
        action={<Link href="/admin/seguimiento" className="rounded-xl border border-white/10 px-3 py-2 text-xs font-medium text-white/75 hover:text-white">Ver seguimiento →</Link>}
      >
      ...
      <ExecutiveCard
        title="Actividad de seguimientos"
        action={<Link href="/admin/seguimiento" className="rounded-xl border border-white/10 px-3 py-2 text-xs font-medium text-white/75 hover:text-white">Ver CRM →</Link>}
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {fm.accionesVencidas > 0 && (
          <div className="mt-4 flex items-center gap-2 rounded-2xl border border-red-500/20 bg-red-500/5 px-4 py-3">
            ...
            <p className="text-sm text-red-300">
              ...
              <Link href="/admin/seguimiento" className="underline underline-offset-2 hover:text-red-200">revisar ahora</Link>
            </p>
          </div>
        )}
      </ExecutiveCard>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/seguimiento" className="rounded-lg border border-white/10 px-4 py-2 text-xs font-semibold text-white/55 hover:text-white">
            Ver seguimiento →
          </Link>
        </div>
...
function OperacionTab({ data }: { data: ExecutiveData }) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <SupportCard title="Sin próximas clases" value={String(data.withoutUpcoming)} detail="Prioridad para retención y reagendamiento" href="/admin/seguimiento" />
      </div>
    </div>
  )
}
...
function AlertItem({
  count, label, sublabel, tone, href,
}: { count: number; label: string; sublabel?: string; tone: 'orange' | 'red'; href?: string }) {
  const isRed = tone === 'red'
  const className = `flex items-start gap-3 rounded-2xl border px-4 py-3 transition-colors ${
    isRed ? 'border-red-500/20 bg-red-500/5' : 'border-[#ff7a00]/15 bg-[#ff7a00]/5'
  } ${href ? (isRed ? 'hover:border-red-500/40 hover:bg-red-500/10' : 'hover:border-[#ff7a00]/30 hover:bg-[#ff7a00]/10') : ''}`
  const content = (
    <>
...
function DualLineChartCard({ trend, hasRealData }: { trend: { label: string; billed: number; cobrado: number }[]; hasRealData: boolean }) {
  const width = 760
  const height = 300
  const padding = 34
  ...
  const toPoint = (item: { label: string; billed: number; cobrado: number }, index: number, key: 'billed' | 'cobrado') => ({
    label: item.label,
...
function DonutChart({ items, total, centerLabel }: { items: { label: string; value: number; color: string }[]; total: number; centerLabel: string }) {
...
function LegendList({ items, total, compact = false }: { items: { label: string; value: number; color: string }[]; total: number; compact?: boolean }) {
  return (
    <div className="space-y-3">
      {items.map((item) => (
...
function CurrencyIcon() {
  return <MdAttachMoney className="h-5 w-5" aria-hidden="true" />
}

function CheckCircleIcon() {
  return <MdCheckCircle className="h-5 w-5" aria-hidden="true" />
}

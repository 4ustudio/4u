...
export default function InstructorProfileModal({ name, email, avatarUrl }: Props) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (result.error) { setUploadError(result.error); setPreview(avatarUrl ?? null) }
      else if (result.url) setUploadedUrl(result.url)
    } catch {
  ...
  if (!open) {
    return (
      <button
        ...
        className="flex items-center gap-2 text-xs text-white/55 hover:text-white transition-colors font-roboto border border-white/15 hover:border-white/30 rounded-xl px-3 py-2"
      >
      ...
      >
        ...
        <div className="flex items-center gap-4">
          <div className="relative shrink-0">
            {preview ? (
              <Image src={preview} alt="Avatar" width={72} height={72} unoptimized
          ...
          <div>
            ...
            {uploadError && <p className="text-[10px] text-red-400 font-roboto mt-1">{uploadError}</p>}
        ...
        >
          <div>
            ...
            <input name="name" defaultValue={name} required disabled={isPending} className={inputClass} placeholder="Nombre completo" />
          </div>
          <div>
          ...
          {state.success && (
            <p className="text-green-600 text-xs font-roboto bg-green-50 border border-green-200 rounded-lg px-3 py-2">✓ Perfil actualizado correctamente</p>
          )}
